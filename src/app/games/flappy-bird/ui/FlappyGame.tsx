...
export const FlappyGame: React.FC = () => {
  ...
  function reset() {
    ...
    pipes.current = [
      makePipe(400),
      makePipe(400 + PIPE_DIST),
      makePipe(400 + 2 * PIPE_DIST),
    ];
    setScore(0);
  }
  ...
  useEffect(() => {
    ...
    const drawHUD = () => {
      ...
      if (!running) {
        ...
        ctx.fillText("чтобы взмахнуть", WIDTH / 2, HEIGHT / 2 + 10);
        ctx.textAlign = "start";
      }
    };
    ...
    const step = () => {
      ...
      if (running) {
        ...
        if (pipes.current[0].x + PIPE_WIDTH < -10) {
          ...
          pipes.current.push(makePipe(lastX));
        }
        const b = bird.current;
        if (b.y > GROUND - 12 || b.y < 12) {
          gameOver();
        }
        for (const p of pipes.current) {
          const inX = b.x + 16 > p.x && b.x - 16 < p.x + PIPE_WIDTH;
          const gapTop = p.top;
          const gapBottom = HEIGHT - p.bottom;
      ...
      drawPipes(pipes.current);
      drawBird(bird.current.x, bird.current.y);
      drawHUD();
